...
center, `web` — cracks diverging in a fan from an off-screen impact, `collapse` — diagonal mesh crumbling out of its
...
Thrown-object cracks diverging in a fan from an off-screen impact — rays radiate from a point
behind the frame (`web.dir` / `web.distance`) and fan across the canvas (`mode: 'web'`):

| ![diverging fan](docs/showcase/web.png) |
|---|
...
- `FracturePattern.version` is 7: `web` is a diverging off-screen fan, corner relief splits the
  corner into wedges from its vertex, and title bands lean diagonally — so patterns regenerate
  differently than v4/v5/v6 for the same seed. Pass `corners: false`, `bands: { diagonal: 0 }`,
  `rings: { asymmetry: 0 }` for the older geometry.
- A pane's break-apart direction is `fx.shatter.spread` (`'fall'` / `'slide'` / `'apart'`), with
  an optional pre-fall sideways drift via `fx.shatter.preSpreadPx` (both default to the v0.6 fall).
- To make a slab edge start beyond the frame (so edge pieces fly in on shatter), generate the
  pattern larger than the visible block and clip the block with `overflow: hidden` — the library
  clips geometry to its own `width`/`height`, the host element crops what shows (see the lab's
  horizontal scene).
